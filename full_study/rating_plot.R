...
data$LetterID2 = ifelse(data$LetterID2 == "Letter 1", "Letter~1~(L[F])",
                         ifelse(data$LetterID2 == "Letter 2", "Letter~2~(L[F])",
                                ifelse(data$LetterID2 == "Letter 3", "Letter~3~(L[M])","Letter~4~(L[M])")))
# 
# data$LetterID <- ifelse(data$LetterID == 1, paste(expression("L"[F]*"O", df$Value, sep = ""), df$Value)
# dataSE1 <- data %>%
#   group_by(LetterID) %>%
#   summarise( 
#     n=n(),
...
# ggsave("p1.pdf", width = 7, height = 5)

########################### bootstrapping for trial 1 ratings
lapply(1:1000, function(i){
  boot_ids <- sample(1:nrow(data), size = nrow(data), replace = T)
  data[boot_ids,] -> boot_data
  boot_data %>%
    group_by(LetterID) %>%
    summarise(mean=mean(letter1_rating)) -> tmp
  tmp$boot <- i
  tmp
...
p1 <- ggplot(data, aes(x=letter1_rating, y=factor(LetterID), color=factor(LetterID)))+
  geom_jitter(shape=16,size=1, position=position_jitter(0.1), alpha=(2/3)) +
  geom_pointrange(data=boot_mu_CI, aes(xmin = lower, xmax = upper,x = mu, y = LetterID ),lwd=1,size=0.8 )+
  scale_y_discrete(limits=rev, labels = c(expression(paste("Letter 4 (", "L"[M], ")")), 
                                          expression(paste("Letter 3 (", "L"[M], ")")), 
                                          expression(paste("Letter 2 (", "L"[F], ")")),
                                          expression(paste("Letter 1 (", "L"[F], ")"))))+
  scale_color_manual(values = c("#E69F00", "#CC79A7", "#009E73", "#fb8072")) +
  geom_text(data=boot_mu_CI, aes( x = mu, y = LetterID,label = sprintf("%.2f", mu), size=18), vjust=1.5,color = "black") +
  labs(x="Rating", y="") +
  guides(color="none", size='none')
p1 + theme_bw() + theme(text = element_text(size = 16))
...
########################### bootstrapping for trial 2 ratings
set.seed(1)
...
data %>%
  ...
  geom_jitter(shape=16,size=1, position=position_jitter(0.1), alpha=(2/3)) +
  geom_pointrange(data=boot_mu_CI, aes(xmin = lower, xmax = upper,x = mu, y = Intervention), lwd=1, size=0.8)+
    ...
    guides(size='none') +
  facet_wrap(~LetterID2,ncol=2, nrow=2, labeller = label_parsed)  + theme_bw() +
...
#   guides(size='none')
# p2 + theme_bw() +
#   facet_wrap(~LetterID2,ncol=2, nrow=2)  + theme_bw() + theme(text = element_text(size = 16), legend.position="top",  strip.background = element_blank())
# 
# ggsave("errorbar.pdf", width = 9, height = 6)
